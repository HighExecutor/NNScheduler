...
def init_agent(args):
    global test_wfs, agent_tasks, nodes, state_size, wfs_names, lock
    # agent = args[0]
    test_wfs = args[1]
    agent_tasks = args[2]
    nodes = args[3]
...
def episode(args):
    ei = args[0]
    agent_act = args[1]
    global test_wfs, agent_tasks, nodes, state_size, wfs_names
...
if __name__ == "__main__":
    ...
    agent = actor.DQNAgent(state_size, action_size, name=process_id)
    ...
    lock = Lock()
    parallel = 5
    pool = Pool(processes=parallel, initializer=init_agent, initargs=((agent, test_wfs, agent_tasks, nodes, state_size, wfs_names, lock), ))
    # Learning
    start = timer.time()
    for e in range(0, EPISODES, parallel):

        rewards = list(pool.map(episode, [(ei, agent.act) for ei in range(e, e + parallel, 1)]))
